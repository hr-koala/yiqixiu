...
class theme_admin extends admin {
	...
	function __construct() {
		global $_M;
		parent::__construct();
		$_M[url][own_tem] = $_M['url']['site'].'app/system/admin/templates/web/theme/';
		$this->iniclass = new skinc($_M['form']['met_skin_user'], $_M['lang']);
		$this->iniclass->configlist[] = 'shopv2_skin_user'; 
		$this->iniclass->mobile_configlist[] = 'shopv2_wap_skin_user'; 
	}
	
	function doindex() {
		global $_M;
		$file = $_M['form']['met_skin_user'] ? $_M['form']['met_skin_user'] : $_M['config']['shopv2_skin_user'];
		$this->checktem($file);
		$devices = 0 ;//默认电脑版
		if($_M[form][mobile])$devices = 1 ;
		$tem = DB::get_all("SELECT * FROM {$_M[table][shopv2_skin_table]} WHERE devices = '{$devices}'");//获取当前选用模板
		/*清除预览*/
		...
		if($_M['form']['mobile']){
			...
			$_M['config']['met_skin_user'] = $_M['config']['shopv2_wap_skin_user'];
		}else{
			$_M['config']['met_skin_user'] = $_M['config']['shopv2_skin_user'];
		}
	...
	function doeditor(){
		...
		if($_M['form']['mobile']){
			$stem = DB::get_one("SELECT * FROM {$_M[table][shopv2_skin_table]} WHERE devices = '1' AND skin_file = '{$_M['config']['wap_skin_user']}'");
			if(!$stem){
				$_M['form']['shopv2_wap_skin_user'] = $_M['form']['wap_skin_user'];
				$_M['form']['wap_skin_user'] = $_M['config']['wap_skin_user'];
				
			}else{
				$_M['form']['shopv2_wap_skin_user'] = $_M['form']['wap_skin_user'];
			}
		}else{
			$stem = DB::get_one("SELECT * FROM {$_M[table][shopv2_skin_table]} WHERE devices = '1' AND skin_file = '{$_M['config']['met_skin_user']}'");
			if(!$stem){
				$_M['form']['shopv2_skin_user'] = $_M['form']['met_skin_user'];
				$_M['form']['met_skin_user'] = $_M['config']['met_skin_user'];
			}else{
				$_M['form']['shopv2_skin_user'] = $_M['form']['met_skin_user'];
			}
		}
	...
	protected function template($path){
		...
		if($postion == 'tem'){
			if (M_MODULE == 'admin') {
				if(file_exists(PATH_SYS."admin/templates/web/theme/{$file}.php")){
					return PATH_SYS."admin/templates/web/theme/{$file}.php";
				}else{
