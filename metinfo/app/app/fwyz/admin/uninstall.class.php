...
class uninstall extends admin {
    ...
    public function dodel() {
        ...
        //删除微信设置
        $keywords = DB::get_one("SELECT * FROM {$_M['table']['nwechat_keywords']} WHERE m_name = '{$this->m_name}' AND m_num = '{$this->m_name}'");
        if($keywords['id'] != ''){
            DB::query("DELETE FROM {$_M['table']['nwechat_keywords']} WHERE id = {$keywords['id']} AND m_num = '{$this->m_name}'");
        }
    }
    ...
    private function zdysql() {
        global $_M;
        $zdy    = array('code','info','record','parameter','list','plist');
        foreach ($zdy   as $val){
