...
define('PROOTPATH', substr(dirname(__FILE__), 0, -20));
	...
	$met_index_type = $sql->get_one("SELECT * FROM $met_config WHERE name='met_index_type' and lang='metinfo'");
	$met_index_type = $met_index_type['value'];
	$lang=($_GET['lang']=="")?$met_index_type:$_GET['lang'];

if($cmodule == 'product_index' || $cmodule == 'product_list'){
...
$query = "SELECT value FROM {$tablepre}config WHERE lang='{$lang}' AND name='shopv2_open'";
