...
class CustomController extends Controller{
   ...
   // 2015-6-30 
	public function statSum(){
		...
		$where['userid']  = intval(session('userid'));
		$where['is_import']  = 0;		
		$unImp=$model->where($where)->count();
		$unImp=$unImp?intval($unImp):0;
		...
		echo '{"success":true,"code":200,"msg":"操作成功","obj":{"unImp":'.$unImp.',"count":'.$count.'},"map":null,"list":null}';
	}
	...
	public function detail(){
		...
		$vo=M('customer')->where($wheredetail)->find();		 
		if($vo){			
			$info='"id":'.$vo["id"].',
			"name":"'.$vo["name"].'",
			"sex":"'.$vo["sex"].'",
	...
	public function delete(){	
		if($_GET['id'] || $_GET['ids']){
			if($_GET['id']){ 
				...
				$wheredetail  =" id in (".I('get.ids',0).")";
			}
			$vo=M('customer')->where($wheredetail)->delete();
			echo '{"success":true,"code":200,"msg":"操作成功","obj":null,"map":null,"list":null}';	  //
		}else{
			echo '{"success":false,"code":100,"msg":"操作失败","obj":null,"map":null,"list":null}';
			}		
