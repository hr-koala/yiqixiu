...
function virifylocal(){
	 ...
	 define('VIRIFY',   true); 	
	define('HTML_VESION',   'index3_1_ka'); 	
}
function aboutaa(){
	virifylocal();
}
...
function checkAllow_nums(){
	...
	if($count>=$allowNum){
		...
		echo '{"success":true,"code":1006,"msg":"您允许创建的场景次数为'.$allowNum.'，请联系管理员增加。","obj":null,"map":null,"list":null}';
...
function upfilesClass($type=1){
	...
	if($type==1){
		...
		$where=array('type'=>'tpType'); 

		//array( 106=> '图标',107=>'动画' ,105=>'节日', 103=>'风格', 102=>'企业',101=>'行业',104=>'个人');



	}elseif($type==2){

		$where=array('type'=>'musType'); 
...
function deal_xd_new($datainfo,$other_arr=array()){	

	

	$where['sceneid_bigint'] = $other_arr['id'];

	$sceneInfo=M('Scene')->where($where)->field('is_payxd','is_payloadlog','property')->find();	
	$str = $sceneInfo['property'];									
	$property = json_decode($str,true);
	
		

	$is_payxd=$sceneInfo['is_payxd'];

	$is_payloadlog=$sceneInfo['is_payloadlog'];

	

	if($other_arr['loadingLogo'] && $is_payloadlog<1 ){

		$qi_ad_xd=C('loadingLogo_qi_xd') ? intval(C('loadingLogo_qi_xd')): 100;

		deal_xd_sub($datainfo,$qi_ad_xd,$other_arr['id'],'加载logo设置','logo');
		$datainfo['is_payloadlog'] = 1;

	}

	$tip='(完全去除)';
		if($property['hideEqAd']){
			
			return $datainfo;
		}
		if($other_arr['property']['eqAdType']==1){

			return $datainfo;	

		}elseif($other_arr['property']['eqAdType']==2){

			$qi_ad_xd= C('guding_qi_xd') ? intval(C('guding_qi_xd')): 15;

			$tip='(固定底标)';

			//return $datainfo;

		}elseif($other_arr['property']['eqAdType']==3){

		   $qi_ad_xd=C('custome_qi_xd') ? intval( C('custome_qi_xd')): 30;
		...
		$qi_ad_xd=C('qi_ad_xds') ? intval(C('qi_ad_xds')):100;

		}		

	

	if($is_payxd<$qi_ad_xd && $property['adSpend'] != 100){

		

		 deal_xd_sub($datainfo,$qi_ad_xd,$other_arr['id'],'去除尾页版权'.$tip);
		if($datainfo['property']){
				$str=str_replace('\\"','"',$datainfo['property']);									
				$datainfo['property']=json_decode($str,true);				
				$datainfo['property']['adSpend'] = 100;
				
			}
		$datainfo['property'] = json_encode($datainfo['property']);
			
	}

	//$qi_ad_xd= C('qi_ad_xds'); //M('sys')->order('id asc')->getField('qi_ad_xds');			

	//$qi_ad_xd=$qi_ad_xd?intval($qi_ad_xd):100;
...
function deal_xd_sub(&$datainfo,$qi_ad_xd,$sceneid,$tip='去除尾页版权',$biztitle='去尾页'){
	...
	if($xd<$qi_ad_xd){
		...
		if($biztitle=="logo"){
			...
			$update_scene=array("is_payloadlog"=>1);

		}else{

			$update_scene=array('is_payxd'=>$qi_ad_xd);
		...
		if($re){
			...
			M('Scene')->where($where)->save($update_scene);	
...
function deal_sceneprice($other_arr){
	...
	if($xd<$qi_ad_xd){
		...
		if($re){
			...
			//M('Scene')->where($where)->save(array('is_payxd'=>$qi_ad_xd));	
		...
		$adddata=array('userid_int'=>session('userid'),
			...
			'sceneid'=>$other_arr['sceneid_bigint'],

			'remark'=>'购买模板：【'.$other_arr['scenename_varchar'].'】',
		...
		\Think\Log::write(D('')->getLastSql()."\n   update:".var_export($adddata,true)."\n");
...
function login_post($cookie) { 
    $post = array ('username' => 'yyh5web@qq.com',
            'password' => '121212',
			'rememberMe'=>false
            ); 
