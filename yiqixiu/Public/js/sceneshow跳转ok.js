...
! function($) {
    ...
    function parseJson(a) {
        ...
        var b = a.list,
            ...
            d = !1,
            e = a.obj.image;
        if (b.length <= 0) return alert("此场景不存在"), void(window.location.href = "http://www.xadfsh.net");
        customLastPage = !1, appendLastPage(a, b), bindShare(a), document.title = a.obj.name, $("#metaDescription").attr("content", a.obj.name + ", " + a.obj.description + ", 由桂林地方生活免费移动场景应用自营销管家提供技术支持"), $(".scene_title").html(a.obj.name);
    ...
    var url, preview, mobileview, pageMode, ad = 0,
        customLastPage = !1,
        scriptLoaded = {};
    url = /[http|https]:\/\/.*\/v-/.test(window.location.href) ? window.location.href.split("/v-")[1] : window.location.href.split("?m=")[1];
